...
window.addEventListener("load", () => {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      getCoordinatesURL(position.coords.latitude, position.coords.longitude);
    });
  } else {
...
function getCoordinatesURL(lat, long) {
  const coordinatesURL = `http://api.openweathermap.org/geo/1.0/reverse?lat=${lat}&lon=${long}&limit=1&appid=${key}`;
  console.log(coordinatesURL);
  getCityName(coordinatesURL);
}
...
async function getWeatherData(data) {
  ...
  let response = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${data[0].name}&APPID=${key}`)
  .then((response) => response.json())
